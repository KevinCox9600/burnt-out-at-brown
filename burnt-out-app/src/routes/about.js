export default function About() {
  const iconSrc = "data:image/png;base64,";
  return (
    <main class="container pt-1">
      <div class="row">
        <div class="col p-3">
          <h2>Our Mission</h2>
          <p>
            Burnt out at Brown is for students looking to optimize their work-life balance.
            We enable this by sorting courses from least to most time consuming.
          </p>
          <div class="d-flex flex-column align-items-center pt-4">
            <img src="https://cdn-icons-png.flaticon.com/512/4171/4171624.png" style={{ "width": "70%" }}></img>
            <a class="pt-3" href="https://www.flaticon.com/free-icons/quality-of-life" title="quality of life icons">Quality of life icons created by Eucalyp - Flaticon</a>
          </div>
        </div>
        <div class="col p-3">
          <h3>Who we are</h3>
          <p>
            We are a team of 4 Brown students who created this website to give students the
            ability to view which courses have the least workload.
          </p>
          <div class="row pt-2">
            <figure class="col text-center">
              <img src={iconSrc}></img>
              <figcaption>Kevin</figcaption>
            </figure>
            <figure class="col text-center">
              <img src={iconSrc}></img>
              <figcaption>Jared</figcaption>
            </figure>
          </div>
          <div class="row">
            <figure class="col text-center">
              <img src={iconSrc}></img>
              <figcaption>Isabelle</figcaption>
            </figure>
            <figure class="col text-center">
              <img src={iconSrc}></img>
              <figcaption>Leyton</figcaption>
            </figure>
          </div>
        </div>
      </div>
    </main >
  );
}